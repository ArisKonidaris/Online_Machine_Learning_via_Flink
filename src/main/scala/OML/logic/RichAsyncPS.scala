...
import org.apache.flink.api.common.state.{AggregatingState, AggregatingStateDescriptor, ListState, ListStateDescriptor, ValueState, ValueStateDescriptor}
...
class RichAsyncPS extends RichPSLogic[workerMessage, ControlMessage] {
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    started = getRuntimeContext.getState(
      new ValueStateDescriptor[Boolean]("processed_data", createTypeInformation[Boolean], false))
